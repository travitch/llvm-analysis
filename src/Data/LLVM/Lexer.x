...
mkAnonLabel s = TLabel $ readText $ T.takeWhile isDigit $ T.drop 10 s
  where isDigit c = c >= '0' && c <= '9'

-- Helpers for the simple literals
mkIntLit s = TIntLit $ readText s
mkFloatLit s = TFloatLit $ readText s
-- Drop the first pfxLen characters (0x)
mkHexFloatLit pfxLen s = TFloatLit $ wordToDouble $ readText s'
  where s' = "0x" `mappend` (T.drop pfxLen s)
-- Strip off the leading c and then unquote
mkStringConstant = TStringLit . unquote . T.tail
mkMetadataString = TMetadataString . unquote . T.tail

readText :: (Read a) => Text -> a
readText = read . T.unpack

-- Discard "cc "
mkNumberedCC s = TCCN $ readText $ T.drop 3 s

-- Extract part between parens (TFAAlignStack Int)
mkAlignStack s = TFAAlignStack $ readText s'
  where s' = T.drop 11 $ T.init s

-- Types
mkTypeUpref s = TUprefT $ readText $ T.tail s
mkIntegralType s = TIntegralT $ readText $ T.tail s

mkAddrSpace s = TAddrspace $ readText s'
...
-- A wrapper based on the bytestring wrapper.  Use Text instead
